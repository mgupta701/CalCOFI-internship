...
bottle_heat <- bottle_filter %>% 
  select(c("quarter","year","oxygen"))
...
heat_map <- ggplot(bottle_heat, aes(year,quarter)) + 
  geom_tile(aes(fill = oxygen),
            colour = "white", 
            na.rm = TRUE) +
  scale_fill_gradient(low = col1, high = col2) +  
  guides(fill = guide_legend(title = "Oxygen Levels")) +
  theme_bw() + theme_minimal() + 
  labs(title ="Oxygen Levels From 1949-2020 over 4 quarters",
       x = "Year", 
       y = "Quarter")+
...
heat_map <- ggplot(hm_data, aes(year,quarter)) + 
  geom_tile(aes(fill = oxygen_perc),
            colour = "white", 
            na.rm = TRUE) +
  scale_fill_gradient(low = col1, high = col2) +  
  guides(fill=guide_legend(title="Oxygen Levels")) +
...
heat_map2 <-ggplot(hm_data, aes(year,quarter)) + 
  geom_tile(aes(fill = oxygen_perc),colour = "white", na.rm = TRUE) +
  scale_fill_gradientn(colours = c("red", "black", "blue"), 
                       breaks= c(3,3.5,4),
                       labels = c("3 mg/L ","3.5 mg/L","4 mg/L")) +
  guides(fill = guide_legend(title="Oxygen Levels")) +
  theme_bw() + theme_minimal() + 
...
median_heatmap_quarterly <- plot_ly(x = hm_data$year, 
                                    y = hm_data$quarter,
                                    z = hm_data$oxygen_perc, 
                                    type = "heatmap",
                                    colors = "Blues", 
...
median_heatmap_yearly <- plot_ly(x = hm2_data$year, 
                                 y = hm2_data$y, 
                                 z = hm2_data$oxygen_perc, 
                                 type = "heatmap", 
                                 colors ="RdYlBu", 
