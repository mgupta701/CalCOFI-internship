...
oxy_ts_plot_percentile <- function(n_ranges, percentile, date_min, date_max){
  ...
  z %>% 
    ...
    labs(title = paste(percentile,
                       "th Percentile Oxygen Across All Stations Over Time"), 
         x = "Date", 
         y = "Oxygen (mL/L)", 
         color = "Depth Range (m)", 
         shape = "Quarter") +
...
oxy_ts_plot_percentile(5, 5, '1970-06-14', '2020-01-26')
...
oxy_ts_plot_sd <- function(n_ranges, date_min, date_max){
  ...
  z %>% 
    ...
    geom_point(na.rm=T) +
    geom_line(linetype='dashed') +
    labs(title = "Standard Deviation in Oxygen Across All Stations Over Time", 
         x = "Date", 
         y = "Oxygen (mL/L)", 
         color = "Depth Range (m)", 
         shape = "Quarter") +
